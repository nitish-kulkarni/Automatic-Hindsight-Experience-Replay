...
for curr_path in paths:
    if not os.path.isdir(curr_path) or 'FR_0.1_full' in curr_path :
    ...
    if replay_strategy == 'generated_k' and params['LAMBDA'] not in done:
        x = params['LAMBDA']
...
for env_id in sorted(data.keys()):
    ...
    xs = [ math.log10(x[0]) for x in data[env_id]]
    ys = [ x[1] for x in data[env_id]]
    err = [ x[2] for x in data[env_id]]
    y_maxs = [ x[3] for x in data[env_id]]

    plt.errorbar(xs,ys,yerr=err,marker='o', linestyle='-', capsize=3, elinewidth=0.5)
    plt.title(env_id)
    plt.xlabel('Log Lambda (Base 10)')
    plt.ylabel('Success Rate')
    plt.legend()
    plt.savefig(os.path.join(args.dir, 'fig_GG_mean_{}.pdf'.format(env_id)))
    ...
    plt.legend()
    plt.savefig(os.path.join(args.dir, 'fig_GG_max_{}.pdf'.format(env_id)))